...
const projects = [
  ...
  { id: 7, title: 'ARTICLES', link: 'https://medium.com/@gvemuru1' },
  { id: 8, title: 'RESUME', isResume: true },
...
const ProjectItem = ({ project, index, onHireMeClick, showContactDetails }) => {
  ...
  return (
    ...
    >
      <h2
        className="no-outline text-white text-4xl sm:text-6xl md:text-7xl lg:text-8xl xl:text-9xl 2xl:text-10xl font-black leading-none cursor-pointer transform sm:-skew-x-18 md:-skew-x-24 transition-all duration-150 outline-none group-hover:text-transparent focus:text-transparent text-right whitespace-nowrap"
        tabIndex={0}
        style={{ 
      ...
      >
        {project.link ? (
          project.link.startsWith('http') ? (
            ...
            <Link 
              to={project.link.replace('.html', '').replace('/index', '/')} 
              className="no-outline" 
              style={{ 
                color: 'inherit', 
                textDecoration: 'none',
...
const ProjectsTimeline = () => {
  ...
  return (
    ...
    >
      ...
      >
        <div 
          className="transform skew-x-24 flex flex-col items-end"
          style={{ 
            WebkitTapHighlightColor: 'transparent',
            WebkitTouchCallout: 'none',
            WebkitUserSelect: 'none',
